...
const Viewfinder = {
  ...
  toggleRecording: function () {
    if (!this.isRecording) {
      // Start recording
      this.recordChunks = [];
      this.mediaStream.getVideoTracks()[0].addEventListener('dataavailable', this.handleRecordData);
      this.recordButton.textContent = 'Stop';
      this.isRecording = true;
    } else {
      // Stop recording
      this.mediaStream.getVideoTracks()[0].removeEventListener('dataavailable', this.handleRecordData);
